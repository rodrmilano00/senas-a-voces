...
// Soporta dos firmas:
//   frameInfo(fingerStates, landmarks) — formato original (una mano)
//   frameInfo(landmarksRight, landmarksLeft) — formato dos manos (cuando el primer arg es array de landmarks)
export function frameInfo(arg1, arg2) {
  // Detectar si arg1 es un array de landmarks (formato dos manos)
  if (Array.isArray(arg1) && arg1.length >= 21) {
    const lmsR = arg1;
    const lmsL = arg2;
    const fsR = computeFingerStates(lmsR);
    const ff = featureFromFingerStates(fsR);
    if (!ff) return null;
    const wrist = lmsR[0];
    const m = lmsR[9];
  ...
  // Formato original: frameInfo(fingerStates, landmarks)
  const fingerStates = arg1;
  const landmarks = arg2;
