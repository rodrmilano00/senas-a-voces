...
class TrainingDataLoader {
  ...
  Future<List<List<Landmark>>> _loadFrames(String path) async {
    try {
      final raw = await rootBundle.loadString(path);
      final arr = jsonDecode(raw) as List;
      final frames = <List<Landmark>>[];
      for (final f in arr) {
        final m = f as Map<String, dynamic>;
        final lms = m['landmarks'];
        if (lms is List && lms.length >= 21) {
          frames.add(lms
              .map((p) => Landmark.fromMap(Map<dynamic, dynamic>.from(p as Map)))
              .toList());
        }
      }
      return frames;
